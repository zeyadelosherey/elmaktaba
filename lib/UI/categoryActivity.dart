...
Future<BooksModel> fetchBooks(String id) async {
  final response =
  await http.get('https://androidtesst.000webhostapp.com/api/getbooks.php?id=${id}');
...
class categoryActivity extends StatefulWidget{
  ...
  categoryActivity({Key key, @required this.uid , @required this.title}) : super(key: key);

  @override
  categoryState createState() => categoryState();
}

class categoryState extends State<categoryActivity>{
  Future<BooksModel> booksmodel;

  @override
  void initState() {
    super.initState();
    booksmodel = fetchBooks(widget.uid) ;
  }
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.white,
        appBar: AppBar(iconTheme: IconThemeData(color: Colors.black), backgroundColor: Colors.white , centerTitle: true,title: Text(widget.title , style:  TextStyle(fontWeight: FontWeight.bold , color: Colors.black , fontSize: 18),), elevation: 2,),
        body:Container(color: Colors.white,

    child:
    FutureBuilder<BooksModel>(
        future: booksmodel,
